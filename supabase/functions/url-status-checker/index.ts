 ...
 */ async function logUrlStatus(supabase, url, shouldLog = true) {
  if (!shouldLog) {
...
Deno.serve(async (req)=>{
  ...
  const { data: stockCodeData, error: stockCodeError } = await supabase.from("stock_code").select("code").order("updatedAt", {
    ascending: false
  }).limit(1).single();
  ...
  await Promise.all(urlsToCheck.map((url)=>{
