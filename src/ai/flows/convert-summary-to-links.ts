 ...
 * - ConvertSummaryToLinksInput - The input type for the function.
 * - ConvertSummaryToLinksOutput - The return type for the function.
 */

import {z} from 'genkit';
import {GEMINI_API_KEY} from '@/lib/constants';

export const ConvertSummaryToLinksInputSchema = z.object({
  summaryHtml: z.string().describe('The HTML summary to convert.'),
});
export type ConvertSummaryToLinksInput = z.infer<
  typeof ConvertSummaryToLinksInputSchema
>;

export const ConvertSummaryToLinksOutputSchema = z.object({
  linkedSummaryHtml: z
    .string()
    .describe('The HTML summary with embedded Google search links.'),
});
export type ConvertSummaryToLinksOutput = z.infer<
  typeof ConvertSummaryToLinksOutputSchema
>;

export async function convertSummaryToLinks(
  input: ConvertSummaryToLinksInput
): Promise<ConvertSummaryToLinksOutput> {
  if (!GEMINI_API_KEY) {
